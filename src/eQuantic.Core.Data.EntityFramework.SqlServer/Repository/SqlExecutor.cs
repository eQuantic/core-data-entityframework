...
public abstract class SqlExecutor : ISqlExecutor, IAsyncSqlExecutor, IDisposable
{
    ...
    /// <summary>
    ///     Gets the query procedure using the specified name. Parameter values are emitted as named
    ///     placeholders matching the <see cref="DbParameter" />s created by <see cref="SetCommand" />,
    ///     so the values are never interpolated into the SQL text.
    /// </summary>
    /// <param name="name">The name</param>
    /// <param name="config">The configuration.</param>
    /// <returns>The string</returns>
